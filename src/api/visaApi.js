...
async function verifierNumero(numero) {
    return requestJson(`/demandes/verifier-numero/${encodeURIComponent(numero)}`, 'Numéro introuvable ou invalide')
}
...
async function getTypesDemande() {
    return requestJson('/types_demande', 'Impossible de charger les types de demande')
}
...
async function getSituationsFamiliales() {
    return requestJson('/situations_familiales', 'Impossible de charger les situations familiales')
}
...
async function getPiecesComplementaires(typeVisaId) {
    ...
    for (const path of paths) {
        try {
            const response = await fetch(`${API_URL}${path}`)
            if (!response.ok) {
                lastError = new Error('Impossible de charger les pièces complémentaires')
                continue
            }
...
async function creerDemande(formData) {
    const response = await fetch(`${API_URL}/demande/form`, {
        method: 'POST',
        body: formData,
    })
